...
import scrapy
import json
from yaoqi.yaoqi.agent_helper import get_random_agent
from yaoqi.yaoqi.items import YaoqiItem
...
class ComicSpider(scrapy.Spider):
    ...
    def start_requests(self):
        data = {'data[group_id]': 'no', 'data[theme_id]': 'no', 'data[is_vip]': 'no', 'data[accredit]': 'no', 
        		'data[color]': 'no', 'data[comic_type]': 'no', 'data[series_status]': 'no', 'data[order]': '2', 'data[read_mode]': 'no'}
